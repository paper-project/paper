...
bool paper_qt::eventloop_processor::event (QEvent * event_a)
{
	assert(dynamic_cast <paper_qt::eventloop_event *>(event_a) != nullptr);
	static_cast <paper_qt::eventloop_event *> (event_a)->action ();
	return true;
}

paper_qt::eventloop_event::eventloop_event (std::function <void ()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

paper_qt::self_pane::self_pane (paper_qt::wallet & wallet_a, paper::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
your_account_label (new QLabel ("Your Paper account:")),
...
void paper_qt::self_pane::refresh_balance ()
{
	paper::transaction transaction (wallet.node.store.environment, nullptr, false);
	std::string balance;
	paper::amount (wallet.node.ledger.account_balance (transaction, wallet.account) / wallet.rendering_ratio).encode_dec (balance);
	wallet.self.balance_label->setText (QString ((std::string ("Balance: ") + balance).c_str ()));
}

paper_qt::accounts::accounts (paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void paper_qt::accounts::refresh ()
{
    model->removeRows (0, model->rowCount ());
	paper::transaction transaction (wallet.wallet_m->store.environment, nullptr, false);
    for (auto i (wallet.wallet_m->store.begin (transaction)), j (wallet.wallet_m->store.end ()); i != j; ++i)
    {
        QList <QStandardItem *> items;
        paper::public_key key (i->first);
        std::string balance;
		paper::amount (wallet.node.ledger.account_balance (transaction, key) / wallet.rendering_ratio).encode_dec (balance);
...
paper_qt::import::import (paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
paper_qt::history::history (paper::ledger & ledger_a, paper::account const & account_a, paper::uint128_t const & rendering_ratio_a) :
model (new QStandardItemModel),
view (new QTableView),
ledger (ledger_a),
account (account_a),
...
namespace
{
class short_text_visitor : public paper::block_visitor
{
public:
	short_text_visitor (MDB_txn * transaction_a, paper::ledger & ledger_a, paper::uint128_t const & rendering_ratio_a) :
	...
	void send_block (paper::send_block const & block_a)
	{
		...
		std::string balance;
		paper::amount (amount / rendering_ratio).encode_dec (balance);
...
void paper_qt::history::refresh ()
{
	paper::transaction transaction (ledger.store.environment, nullptr, false);
	model->removeRows (0, model->rowCount ());
...
paper_qt::block_viewer::block_viewer (paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void paper_qt::block_viewer::rebroadcast_action (paper::uint256_union const & hash_a)
{
	auto done (true);
	paper::transaction transaction (wallet.node.ledger.store.environment, nullptr, false);
...
paper_qt::status::status (paper_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	active.insert (paper_qt::status_types::nominal);
	set_text ();
}

void paper_qt::status::erase (paper_qt::status_types status_a)
{
	assert (status_a != paper_qt::status_types::nominal);
	auto erased (active.erase (status_a)); (void)erased;
	set_text ();
}

void paper_qt::status::insert (paper_qt::status_types status_a)
{
	assert (status_a != paper_qt::status_types::nominal);
...
void paper_qt::status::set_text ()
{
	wallet.status->setText (text ().c_str ());
}

std::string paper_qt::status::text ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
	{
		case paper_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case paper_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case paper_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case paper_qt::status_types::nominal:
...
paper_qt::wallet::wallet (QApplication & application_a, paper::node & node_a, std::shared_ptr <paper::wallet> wallet_a, paper::account const & account_a) :
rendering_ratio (paper::Mpaper_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
active_status (*this)
{
	...
	send_account->setPlaceholderText (paper::zero_key.pub.to_base58check ().c_str ());
    ...
    QObject::connect (send_blocks_send, &QPushButton::released, [this] ()
    {
		...
		{
			paper::uint128_t actual (amount.number () * rendering_ratio);
    ...
    node.observers.push_back ([this] (paper::block const &, paper::account const & account_a)
    {
	...
	node.endpoint_observers.push_back ([this] (paper::endpoint const &)
	{
	...
	node.bootstrap_initiator.observers.push_back ([this] (bool active_a)
	{
		application.postEvent (&processor, new eventloop_event ([this, active_a] ()
		{
			if (active_a)
			{
				active_status.insert (paper_qt::status_types::synchronizing);
			}
			else
			{
				active_status.erase (paper_qt::status_types::synchronizing);
			}
		}));
	});
	refresh ();
...
void paper_qt::wallet::refresh ()
{
	{
		paper::transaction transaction (wallet_m->store.environment, nullptr, false);
...
void paper_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (paper_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (paper_qt::status_types::disconnected);
	}
}

void paper_qt::wallet::push_main_stack (QWidget * widget_a)
{
    main_stack->addWidget (widget_a);
    main_stack->setCurrentIndex (main_stack->count () - 1);
}

void paper_qt::wallet::pop_main_stack ()
{
    main_stack->removeWidget (main_stack->currentWidget ());
}

paper_qt::settings::settings (paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
	...
	new_representative->setPlaceholderText (paper::zero_key.pub.to_base58check ().c_str ());
    ...
    {
		paper::transaction transaction (wallet.wallet_m->store.environment, nullptr, true);
	...
	{
		paper::account representative_l;
...
void paper_qt::settings::activate ()
{
    wallet.push_main_stack (window);
}

void paper_qt::settings::update_locked ()
{
	paper::transaction transaction (wallet.wallet_m->store.environment, nullptr, false);
	if (wallet.wallet_m->store.valid_password (transaction))
	{
		password->clear ();
		wallet.active_status.erase (paper_qt::status_types::locked);
	}
	else
	{
		wallet.active_status.insert (paper_qt::status_types::locked);
	}
}

paper_qt::advanced_actions::advanced_actions (paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this] ()
	{
		...
		auto error (paper::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void paper_qt::advanced_actions::refresh_peers ()
{
    QStringList peers;
...
void paper_qt::advanced_actions::refresh_ledger ()
{
    ...
    {
        QList <QStandardItem *> items;
        items.push_back (new QStandardItem (QString (paper::block_hash (i->first).to_base58check ().c_str ())));
		auto hash (paper::account_info (i->second).head);
		std::string balance;
		paper::amount (wallet.node.ledger.balance (transaction, hash) / wallet.rendering_ratio).encode_dec (balance);
...
paper_qt::block_entry::block_entry (paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
    ...
    QObject::connect (process, &QPushButton::released, [this] ()
    {
        ...
        try
        {
            ...
            auto block_l (paper::deserialize_block_json (tree));
            if (block_l != nullptr)
            {
...
paper_qt::block_creation::block_creation (paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void paper_qt::block_creation::deactivate_all ()
{
    account_label->hide ();
    account->hide ();
    source_label->hide ();
...
void paper_qt::block_creation::activate_send ()
{
    account_label->show ();
    account->show ();
    amount_label->show ();
...
void paper_qt::block_creation::activate_receive ()
{
    source_label->show ();
    source->show ();
}

void paper_qt::block_creation::activate_open ()
{
    source_label->show ();
    source->show ();
...
void paper_qt::block_creation::activate_change ()
{
    account_label->show ();
    account->show ();
    representative_label->show ();
            ...
            if (!error)
            {
                ...
                if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
                {
                    ...
                    if (amount_l.number () <= balance)
                    {
                        ...
                        assert (!error);
                        paper::send_block send (info.head, destination_l, balance - amount_l.number (), key, account_l, wallet.wallet_m->work_fetch (transaction, account_l, info.head));
                        key.clear ();
                        std::string block_l;
...
void paper_qt::block_creation::create_receive ()
{
    paper::block_hash source_l;
    ...
    if (!error)
    {
        ...
        if (!wallet.node.store.pending_get (transaction, source_l, receivable))
        {
            ...
            if (!error)
            {
                ...
                if (!error)
                {
                    paper::receive_block receive (info.head, source_l, key, receivable.destination, wallet.wallet_m->work_fetch (transaction, receivable.destination, info.head));
                    key.clear ();
                    std::string block_l;
...
void paper_qt::block_creation::create_change ()
{
    paper::account account_l;
    auto error (account_l.decode_base58check (account->text ().toStdString ()));
    if (!error)
    {
        ...
        if (!error)
        {
            ...
            if (!error)
            {
                ...
                if (!error)
                {
                    paper::change_block change (info.head, representative_l, key, account_l, wallet.wallet_m->work_fetch (transaction, account_l, info.head));
                    key.clear ();
                    std::string block_l;
...
void paper_qt::block_creation::create_open ()
{
    paper::block_hash source_l;
    auto error (source_l.decode_hex (source->text ().toStdString ()));
    if (!error)
        ...
        if (!error)
        {
            ...
            if (!wallet.node.store.pending_get (transaction, source_l, receivable))
            {
                ...
                if (error)
                {
                    ...
                    if (!error)
                    {
                        paper::open_block open (source_l, representative_l, receivable.destination, key, receivable.destination, wallet.wallet_m->work_fetch (transaction, receivable.destination, receivable.destination));
                        key.clear ();
                        std::string block_l;
