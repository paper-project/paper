...
public:
	...
	{
		paper::random_pool.GenerateBlock (wallet.bytes.begin (), wallet.bytes.size ());
...
int main (int argc, char * const * argv)
{
	...
	paper::landing_config config;
	{
		std::ifstream config_stream;
		config_stream.open ((working / "config.json").string ());
		if (!config_stream.fail ())
		{
			paper::landing_config loaded_config (config_error, config_stream);
	...
	if (!config_error)
	{
		paper::landing_store store;
		{
			...
			{
				paper::landing_store loaded_store (config_error, store_stream);
				store = loaded_store;
			}
			else
			{
		...
		if (!config_error)
		{
			...
			if (!init.error ())
			{
				...
				paper::landing landing (*node, wallet, store, working / "landing.json");
